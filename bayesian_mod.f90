...
contains
   subroutine RunMonteCarlo(taskid, numprocs, arg)
      ...
      if (masterproc .and. (.not.DEBUG)) then
         if (maxid<NMAXSAMPLE) then
            ...
            write(fullname, "(A,I0,A)") "bLakeJob", lake_range(1), ".sub"
            print *, fullname
           call GetFullFileName(fullname, script)
      ...
      ! Uncomment if want to output all calibration results
   ...
   subroutine MonteCarloSimulation(sampleId, sample, vars, weights, &
      ...
      ! Uncomment if want to output all calibration results
      !call ArchiveModelOutput(sampleId, time)
      if (error==0) then
